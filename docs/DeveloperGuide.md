...
`Username` uses exact string equality, so username uniqueness is case-sensitive.
...
* `TutorSlotCommand` â€” Takes an `Index` and a `TimeSlot`. On execution, it:
    ...
    3. Checks for overlapping time slots on the same day (exact duplicates are a subset of overlap).
       For example, if an existing slot is `mon-10-12`, then `mon-11-13`, `mon-10-12`, and `mon-10-11` are rejected.
       Boundary-touching slots such as `mon-10-12` and `mon-12-14` are allowed because they are not treated as overlap.
...
* `ExportCommandParser` â€” Parses user input with optional file path prefix `f/`. If no file path is provided, uses the
...
1. User shows a list containing the target teaching staff member (e.g. using `staffslist` or `list`).
2. Doritus shows the requested list.
3. User identifies the target teaching staff member and notes their index.
...
* 4c. The time slot overlaps with an existing slot for this staff member.
    ...
    * 4c1. Doritus shows the error message `This time slot overlaps with an existing slot for this teaching staff
      member.`
